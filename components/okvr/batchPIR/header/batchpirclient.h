...
public:
    ...
    void set_map(std::unordered_map<string, uint64_t> map);
    ...
    vector<vector<uint64_t>> getCuckooTables()
    {
        return cuckooTables;
    }
    ...
    BatchPirParams getBatchPirParams()
    {
        return batchpir_params_;
    };
...
private:
    ...
    vector<vector<uint64_t>> cuckooTables;
