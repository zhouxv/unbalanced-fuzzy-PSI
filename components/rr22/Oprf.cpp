...
namespace volePSI {
Proto RsOprfSender::send(u64 n, PRNG &prng, Socket &chl, u64 numThreads,
                         bool reducedRounds) {
  auto ws = block{};
  auto hBuff = std::array<u8, 32>{};
  auto ro = oc::RandomOracle(32);
  auto pPtr = std::unique_ptr<block[]>{};
  auto pp = std::span<block>{};
  auto subPp = std::span<block>{};
  auto remB = std::span<block>{};
  auto subB = std::span<block>{};
  ...
  pp = std::span<block>(pPtr.get(), mPaxos.size());
  ...
  // RsOprfSender::send(u64n,PRNG&prng,Socket&chl,u64numThreads,boolreducedRounds)
...
void RsOprfSender::eval(std::span<const block> val, std::span<block> output,
                        u64 numThreads) {
...
Proto RsOprfSender::genVole(PRNG &prng, Socket &chl, bool reduceRounds) {
  if (reduceRounds) {
    mVoleSender.mBaseType = oc::SilentBaseType::Base;
    mVoleSender.configure(mPaxos.size());
  }

  return mVoleSender.silentSendInplace(mD, mPaxos.size(), prng, chl);
}

struct UninitVec : std::span<block> {
...
Proto RsOprfReceiver::receive(std::span<const block> values,
                              std::span<block> outputs, PRNG &prng, Socket &chl,
                              u64 numThreads, bool reducedRounds) {
  ...
  auto h = std::span<block>{};
  auto p = UninitVec{};
  auto subP = std::span<block>{};
  auto subC = std::span<block>{};
  auto a = std::span<block>{};
  auto c = std::span<block>{};
  auto fu = macoro::eager_task<void>{};
  auto ii = u64{0};
  auto fork = Socket{};
...
Proto RsOprfReceiver::genVole(u64 n, PRNG &prng, Socket &chl,
                              bool reducedRounds) {
  if (reducedRounds) {
    mVoleRecver.mBaseType = oc::SilentBaseType::Base;
    mVoleRecver.configure(n);
  }
